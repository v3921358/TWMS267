export function help() {
    var text = [
        ...
        `#help - 顯示指令列表`,
        `#ea - 解卡`,
        `#give [-t 角色名] <物品ID> [數量] - 給予道具`,
        `#giveMap [-m 地圖ID] <物品ID> [數量] - 向指定地圖玩家給予道具`,
        `#givePoint [-t 角色名] <cash|point|meso> <數量> - 給予 樂豆|楓點|楓幣`,
        `#map <地圖ID> - 傳送到指定地圖`,
        `#rate <exp|drop> <頻道> <數值> - 設置某個頻道經驗或掉寶倍率`,
        `#spawn <怪物ID> [-count 數量] [-hp 血量] - 召喚怪物`,
        `#setSkill <技能ID> <技能等級> - 獲得技能`,
        `#useSkill <技能ID> <技能等級> - 獲得技能buff`,
        `#maxSkill - 满技能`,
        `#reloadskill - 重载技能`,
        `#ban <角色名> [-withMac true] [-reason 原因]- 封號`,
        `#unban <角色名> - 解封賬號`,
        `#fam - 萌獸圖鑒`,
        `#hide - 隱身`,
        `#invincible - 無敵`,
        `#kick <角色名|-all true> - 指定玩家下線或者全服下線`,
        `#mob - 查看怪物信息`,
        `#notice [-t 類型] <內容> - 發送公告`,
        `#runScript <腳本路徑> - 打開腳本`,
        `#online - 在線玩家數量`,
        `#pos - 查看坐標`,
        `#separation <倍數> - 傷害倍增（分身功能）`,
        `#cooldown - 技能無CD`,
        `#autoAttack - 自動攻擊（過圖自動取消）`,
        `#debug <quests|npcs|items|maps|items|portals|events|reactors|commands|clear|all|show> - 過濾調試信息`,
        `#shutdown <-t 時間(秒)> - 定時關閉服務器`,
        `──────────────────────────`,
    ]
    for (var t of text) {
        player.showSpouseMessage(9, t)
    }
}

export function give(commandLine) {
    switch (commandLine.length) {
        case 2:
            player.gainItem(Number(commandLine[1]), 1)
            break;
        case 3:
            player.gainItem(Number(commandLine[1]), Number(commandLine[2]))
            break;
        case 4:
            if (commandLine[1] == "-t") {
                player.giveItemForPlayerName(commandLine[2], Number(commandLine[3]), 1)
            } else {
        ...
        case 5:
            if (commandLine[1] == "-t") {
                player.giveItemForPlayerName(commandLine[2], Number(commandLine[3]), Number(commandLine[4]))
            } else {
                player.showSpouseMessage(9, "指令格式錯誤")
            }
            break;
...
export function giveItemForMap(commandLine) {
    switch (commandLine.length) {
        case 2:
            player.giveItemForMap(player.getMapId(), Number(commandLine[1]))
            break;
        case 3:
            player.giveItemForMap(player.getMapId(), Number(commandLine[1]), 1)
            break;
        case 4:
            if (commandLine[1] == "-m") {
                player.giveItemForMap(Number(commandLine[2]), Number(commandLine[3]), 1)
            } else {
                player.showSpouseMessage(9, "指令格式錯誤")
            }
            break;
        case 5:
            if (commandLine[1] == "-m") {
                player.giveItemForMap(Number(commandLine[2]), Number(commandLine[3]), Number(commandLine[4]))
            } else {
                player.showSpouseMessage(9, "指令格式錯誤")
            }
            break;
...
export function givePoint(commandLine) {
    switch (commandLine.length) {
        case 3:
            switch (commandLine[1]) {
                case "cash":
                    player.modifyCashShopCurrency(1, Number(commandLine[2]))
                    break
                case "point":
                    player.modifyCashShopCurrency(2, Number(commandLine[2]))
                    break
                case "meso":
                    player.gainMeso(Number(commandLine[2]))
                    break
                default:
                    player.showSpouseMessage(9, "指令格式錯誤")
                    break;
        ...
        case 5:
            if (commandLine[1] == "-t") {
                player.givePointForPlayerName(commandLine[2], Number(commandLine[3]), Number(commandLine[4]))
            } else {
                player.showSpouseMessage(9, "指令格式錯誤")
            }
            break;
...
export function kick(commandLine) {
    switch (commandLine.length) {
        case 2:
            player.KickForPlayerName(false, commandLine[1])
            break
        case 3:
            if (commandLine[1] == "-all" && commandLine[2] === "true") {
                player.KickForPlayerName(true, null)
            } else {
                player.showSpouseMessage(9, "指令格式錯誤")
            }
            break;
...
export function notice(commandLine) {
    switch (commandLine.length) {
        case 2:
            player.broadcastPlayerNotice(9, Numer(commandLine[1]))
            break
        case 4:
            if (commandLine[1] == "-t") {
                player.broadcastPlayerNotice(Numer(commandLine[2]), Numer(commandLine[3]))
            } else {
                player.showSpouseMessage(9, "指令格式錯誤")
            }
            break;
...
export function rate(commandLine) {
    if (commandLine.length == 4) {
        switch (commandLine[1]) {
            case "exp":
                player.gmExpRate(Number(commandLine[2]), Number(commandLine[3]))
                break
            case "drop":
                player.gmDropRate(Number(commandLine[2]), Number(commandLine[3]))
                break
            default:
                player.showSpouseMessage(9, "指令格式錯誤")
        }
...
export function spawn(commandLine) {
    switch (commandLine.length) {
        case 2:
            player.gmSpawn(Number(commandLine[1]), 1, -1)
            break
        case 4:
            if (commandLine[2] == "-count") {
                player.gmSpawn(Number(commandLine[1]), Number(commandLine[3]), -1)
            } else if (commandLine[2] == "-hp") {
                player.gmSpawn(Number(commandLine[1]), 1, Number(commandLine[3]))
            } else {
                player.showSpouseMessage(9, "指令格式錯誤")
            }
            break;
        case 6:
            if (commandLine[2] == "-count" && commandLine[4] == "-hp") {
                player.gmSpawn(Number(commandLine[1]), Number(commandLine[3]), Number(commandLine[5]))
            } else {
                player.showSpouseMessage(9, "指令格式錯誤")
            }
            break
...
export function ban(commandLine) {
    switch (commandLine.length) {
        case 2:
            player.ban(commandLine[1], "", false)
            break
        case 4:
            if (commandLine[2] == "-withMac" && commandLine[3] === "true") {
                player.ban(commandLine[1], "", true)
            } else if (commandLine[2] == "-reason") {
                player.ban(commandLine[1], commandLine[3], false)
            } else {
                player.showSpouseMessage(9, "指令格式錯誤")
            }
            break;
        case 6:
            if (commandLine[2] == "-withMac" && commandLine[3] === "true" && commandLine[4] == "-reason") {
                player.ban(commandLine[1], commandLine[5], true)
            } else {
                player.showSpouseMessage(9, "指令格式錯誤")
            }
            break
...
export function maxSkill() {
    let choice = player.getJob();
    // 最大化技能
    switch (choice) {
        case 100: // subJob 劍士
            player.maxSkills();
