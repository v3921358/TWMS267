...
if (event != null) {
        ...
        var rStat = map.getReactorStateId("b") + 1;
        var rValue = parseInt(player.getQuestRecordEx(42006, "b"));
        if (rValue > rStat) {//获得的数字 大于当前的数字 则进行处理
                player.updateQuestRecordEx(42006, "bc", String(rValue));
                player.showAvatarOrientedRepeat(false, "");
                player.showAvatarOriented("Effect/OnUserEff.img/aquarisTower/success");
                player.updateQuestRecordEx(42006, "b", "0");
                map.setReactorState("b", rValue - 1, 0);
