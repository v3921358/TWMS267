...
import Config.constants.BattleConstants;
import Config.constants.BattleConstants.*;
...
public class Battler implements Serializable {
    ...
    public void setStatus(MonsterStatusEffect mse) {
        ...
        if (ability == PokemonAbility.Insomnia && stat == MonsterStatus.Blind) {
            return;
        }
        if (ability == PokemonAbility.Limber && stat == MonsterStatus.Web) {
            return;
        }
        if (ability == PokemonAbility.MagmaArmor && stat == MonsterStatus.Speed) {
            return;
        }
        ...
        } else if (stat == MonsterStatus.Speed || stat == MonsterStatus.Web) {
            for (int zz = 0; zz < getStatusTurns() - 1; zz++) {
                setMod(PokemonStat.SPEED, decreaseMod(getMod(PokemonStat.SPEED)));
            }
        } else if (stat == MonsterStatus.Weakness) {
            for (int zz = 0; zz < getStatusTurns() - 1; zz++) {
    ...
    private void calculateFamily() {
        ...
        for (Entry<Element, ElementalEffectiveness> e : stats.getElements().entrySet()) {
            if (e.getValue() == ElementalEffectiveness.免疫 || e.getValue() == ElementalEffectiveness.增強) {
