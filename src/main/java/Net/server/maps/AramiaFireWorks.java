...
public class AramiaFireWorks {
    ...
    private void broadcastServer(MapleCharacter c, int itemid) {
        WorldBroadcastService.getInstance().broadcastMessage(MaplePacketCreator.serverNotice(6, itemid, "<頻道 " + c.getClient().getChannel() + "> " + c.getMap().getMapName() + " : The amount of {" + MapleItemInformationProvider.getInstance().getName(itemid) + "} has reached the limit!"));
    }
    ...
    private void spawnItem(MapleMap map) {
        ...
        for (int i = 0; i < Randomizer.nextInt(5) + 10; i++) {
            ...
            int itemId = 4001246; //溫暖的陽光 - 為了讓豆芽快速生長，把溫暖的陽光放到豆芽上吧
            switch (Randomizer.nextInt(15)) {
                case 0:
                case 1:
                    itemId = 3010141; //蛋糕椅子 - 蛋糕師精心製作的蛋糕. 坐在上面每10秒鐘恢復HP,MP各30
                    break;
                case 2:
                    itemId = 3010146; //週年慶水晶楓葉椅子 - 坐在上面時，每10秒恢復HP的楓之谷週年慶水晶楓葉椅子
                    break;
                case 3:
                case 4:
                    itemId = 3010025; //楓葉紀念凳 - 為紀念楓之谷5週年生日而特製的凳子。坐在楓葉樹下會自然而然地回憶起最初的感動。每10秒鐘可恢復HP 35,MP 10
                    break;
            }
            map.spawnAutoDrop(itemId, pos, 0, 0);
        }
    }
    ...
    private void spawnDec(MapleMap map) {
        ...
        for (int i = 0; i < Randomizer.nextInt(10) + 40; i++) {
            ...
            map.spawnAutoDrop(Randomizer.nextInt(15) == 1 ? 4310012 : 4310011, pos, 0, 0);
        }
    }
}
