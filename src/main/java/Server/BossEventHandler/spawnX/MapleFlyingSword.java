...
public abstract class MapleFlyingSword extends MapleMapObject {
    ...
    public FlyingSwordNode setNodes(List<FlyingSwordNode> nodes) {
        this.nodes = nodes;
        return null;
    }
    ...
    public void updateTarget(MapleMap map) {
        if (map.getAllCharactersThreadsafe().size() > 0) {
            MapleCharacter target = map.getAllCharactersThreadsafe().get(Randomizer.nextInt(map.getAllCharactersThreadsafe().size()));
            if (target != null) {
                setTarget(target);
