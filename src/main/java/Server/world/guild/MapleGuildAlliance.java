...
public class MapleGuildAlliance implements Serializable {
    ...
    private static final Logger log = LoggerFactory.getLogger(MapleGuildAlliance.class);
    ...
    public MapleGuildAlliance(int id) {
        ...
        try (Connection con = DatabaseConnectionEx.getInstance().getConnection()) {
            ...
            log.error("[MapleGuildAlliance] 從數據庫中加載公會聯盟信息出錯." + se);
        }
    }
    ...
    public static int createToDb(int leaderId, String name, int guild1, int guild2) {
        ...
        try (Connection con = DatabaseConnectionEx.getInstance().getConnection()) {
            ...
            log.error("[MapleGuildAlliance] 創建公會聯盟出錯." + se);
        }
        return ret;
    }
    ...
    public boolean deleteAlliance() {
        try (Connection con = DatabaseConnectionEx.getInstance().getConnection()) {
            ...
            log.error("[MapleGuildAlliance] 解散公會聯盟出錯." + se);
            return false;
        }
        return true;
    }
    ...
    public void saveToDb() {
        try (Connection con = DatabaseConnectionEx.getInstance().getConnection()) {
            ...
            log.error("[MapleGuildAlliance] 保存公會聯盟出錯." + se);
        }
    }
    ...
    public void setNotice(String newNotice) {
        ...
        broadcast(GuildPacket.getAllianceUpdate(this));
        broadcast(MaplePacketCreator.serverNotice(5, "* 聯盟公告 : " + newNotice));
        saveToDb();
    }
    ...
    public boolean setLeaderId(int c, boolean sameGuild) {
        ...
        guilds[0] = oldGuild;
        if (leaderName != null) {
            broadcast(MaplePacketCreator.serverNotice(5, "* 聯盟公告 : " + leaderName + " 成為了公會聯盟會長."));
        }
